const EventGloEmbed = (() => {
  ...
  const templates = {
    0: (eventData) => {
      ...
      return `<div class="card embed-card" role="button">${getTag(
        eventData
    ...
    1: (eventData) => {
      const startDate = new Date(eventData.startDate);
      return `<div class="card embed-card" role="button"> <div class="card-body">${getTag(
        eventData
  ...
  function getTag(eventData) {
    return `<div class="tag ${eventData.eventType === 0 ? 'free' : 'paid'}">
                    ${eventData.eventType === 0 ? 'Free' : 'Paid'}
            </div>`;
  }
